...
def display_professor_lecture():
    ...
    # Complete HTML presentation from your document (cleaned of Unicode issues)
    html_content = """
    <!DOCTYPE html>
    <html lang="en">
    <head>
    ...
    st.markdown("- Press 'P' for presentation mode (hides navigation)")
    st.markdown("- Press 'N' to toggle presenter notes")
    st.markdown("- Timers automatically start on activity slides")
...
def display_assignment1():
    ...
    with st.expander("Assignment Instructions", expanded=True):
        ...
        **The Five Required Question Types:**
        ...
        ### The Philosophers:
        - **Emile Durkheim** (1858-1917): Religion as social glue
    ...
    if not openai_key:
        st.info("Add your OpenAI API key in the sidebar for dynamic philosopher conversations!")
    ...
    if len(progress_data['questions_asked'][philosopher]) >= 5:
        st.markdown(f"## Essay about {profile['name']}")
...
Clean ASCII version - no Unicode characters that cause syntax errors
"""
...
SLIDES = [
    ...
    },
    {
        ...
        ## Breaking it Down:
        * **Philo (Greek: philos)** = love, affection, friendship
        * **Sophia (Greek: sophia)** = wisdom, skill, deep knowledge
